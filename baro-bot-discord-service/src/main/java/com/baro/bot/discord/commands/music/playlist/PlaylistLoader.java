...
package com.baro.bot.discord.commands.music.playlist;
...
public class PlaylistLoader {
    ...
    private final static String playlistFolder = "Playlists";
    ...
    public void createFolder() {
        try {
            Files.createDirectory(Paths.get(playlistFolder));
        } catch (IOException ignore) {
        }
    }

    public boolean folderExists() {
        return Files.exists(Paths.get(playlistFolder));
    }

    public void createPlaylist(String name) throws IOException {
        Files.createFile(Paths.get(playlistFolder + File.separator + name + ".txt"));
    }

    public void deletePlaylist(String name) throws IOException {
        Files.delete(Paths.get(playlistFolder + File.separator + name + ".txt"));
    }

    public void writePlaylist(String name, String text) throws IOException {
        Files.write(Paths.get(playlistFolder + File.separator + name + ".txt"), text.trim().getBytes());
    }
    ...
    public Playlist getPlaylist(String name) {
        ...
        try {
            if (folderExists()) {
                ...
                Files.readAllLines(Paths.get(playlistFolder + File.separator + name + ".txt")).forEach(str ->
                {
                    String s = str.trim();
                    if (s.isEmpty())
                        return;
